[index.html (2).html](https://github.com/user-attachments/files/27656310/index.html.2.html)
<!DOCTYPE html>
<html lang="es">
<head>
<meta charset="UTF-8">
...
function depositoBadge(r) {
  if(!r.deposito) return '<span style="color:var(--text3)">—</span>';
  const cls = r.estadoPago==='paid'?'badge-paid':r.estadoPago==='pending'?'badge-pending':'';
  return `<span class="badge ${cls}">$${r.deposito.toLocaleString('es-CL')}</span>`;
}
